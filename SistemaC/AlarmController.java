	...
	public static void main(String args[])
	{
		...
		if (em != null)
		{
			...
			*/
			...
			Indicator ai = new Indicator ("AlarmState OFF", mw.GetX(), mw.GetY()+mw.Height());
			...
			while ( !Done )
			{
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					if ( Evt.GetEventId() == 8 )
					{
						...
						if (Evt.GetMessage().equalsIgnoreCase("A1")) // heater on
						{
							AlarmState = true;

						} // if

						if (Evt.GetMessage().equalsIgnoreCase("A0")) // heater off
						{
							AlarmState = false;
				...
				if (AlarmState)
				{
					...
					ai.SetLampColorAndMessage("ROOM OK", 1);
